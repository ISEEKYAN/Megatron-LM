...
from examples.bench.wgrad_dump import wgrad_dump_context
...
def _hash_tensor(tensor: torch.Tensor | None) -> dict[str, Any] | None:
    ...
    if summary is not None:
        ...
        result["summary"] = {
            ...
            "square_sum": float(flat.double().square().sum().item()),
...
def _parameter_probes(handle) -> list[dict[str, Any]]:
    patterns = [
        item.strip()
        for item in os.environ.get("MLITE_CORRECTNESS_PARAM_PROBES", "").split(",")
        if item.strip()
    ]
    if not patterns:
        return []
...
def _weighted_logprob_loss(model_output: dict[str, Any], batch: Any):
    """Deterministic PPO-like linear objective over chosen-token log probabilities."""
    labels = batch["labels"] if isinstance(batch, dict) else batch.labels
    log_probs = model_output.get("log_probs")
    if log_probs is None:
        token_loss = model_output["output_tensor"]
        log_probs = -token_loss.transpose(0, 1).contiguous()
    log_probs = log_probs.reshape(labels.shape)
    active = labels.ne(-100)
    token_index = torch.arange(labels.numel(), device=labels.device).reshape(labels.shape)
    advantages = ((token_index % 17).float() - 8.0) / 8.0
    denominator = active.sum().clamp_min(1)
    loss = -(log_probs.float() * advantages * active).sum() / denominator
...
def run_backend(
    ...
    *,
    ...
    deterministic = os.environ.get("MLITE_CORRECTNESS_NONDETERMINISTIC") != "1"
    if deterministic:
        os.environ["MEGATRON_LITE_DETERMINISTIC"] = "1"
        set_deterministic(cfg.seed)
    else:
        os.environ.pop("MEGATRON_LITE_DETERMINISTIC", None)
    ...
    loss_fn = (
        _weighted_logprob_loss
        if os.environ.get("MLITE_CORRECTNESS_WEIGHTED_LOGPROB") == "1"
        else None
    )
    with rt.train_mode(handle):
        for step in range(session_cfg.steps):
            with _activation_probe_context(
                ...
                with wgrad_dump_context(handle._model, step=step):
                    result = rt.forward_backward(
                        handle,
                        data_iter,
                        loss_fn=loss_fn,
    ...
    return {
        ...
        "parameter_probes": _parameter_probes(handle),
        "metadata": {
