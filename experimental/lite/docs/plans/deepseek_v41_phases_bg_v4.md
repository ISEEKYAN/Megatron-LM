...
uncorrected snapshot's CED denial or differentiable-integer arrows.
The current planning checkout is based on
`26e9bf64faf06be04686f199f8eecbfd07861a9c` and does not contain those specifications.
...
| C4-S: consume A1–A4, all 96085 keys and 87 config leaves; declare canonical owner/alias, header→logical weight binding and protocol contract | C4-I: `model/deepseek_v41/{config.py,__init__.py,lite/model.py,lite/protocol.py,lite/checkpoint.py}` plus `model/registry.py`; compose D modules, backbone, vision and aligner, preserve full module tree in text-only mode | `test_model_binding.py`: nested config reaches actual behavior, every key has a header/store/owner mapping, every active parameter is bound once; construct via registry and run protocol; update backbone then export/reload: backbone changed, MTP bytes unchanged | F2 actual optimizer enumeration; E distributed binding and G1 real-size all-key load |
...
is an acceptance criterion. Multiple microbatches must apply the declared
1/num_microbatches scale once through the real pre-forward protocol hook;
...
| E5v-S: F3 multimodal stage transition restart | E5v-I: visual/external encoder optimizer and scheduler restore | Slurm `test_restart_vision.py`: same-total-step trajectory crossing freeze/unfreeze, replicas and LR/state initialization restored | G1 |
...
| Accumulated gradients | Contributions coalesced at declared owner, tagged by step/microbatch; freed only after atomic successful step | Accumulation/storage/reduction dtype and deterministic order remain OPEN |
| Momentum and any scale cache | Optimizer shard owns persistent M; optional cache is reconstructible, not warm-start state | Momentum dtype/rounding remain OPEN; M is checkpointed |
...
| F3-S: A4 visual schedule, preprocessing, spans, external-encoder sync and gradient contract | F3-I: `model/deepseek_v41/lite/vision.py` and protocol schedule | `test_vision.py` then Slurm `test_vision_schedule.py`: official processor forward, independent differentiable serial baseline, real external copies/sync, trainable exceptions and transitions | E5v/G1 |
...
an `OPEN:` prefix in the algorithm column. Keep the following complete known
ledger and fail if another unresolved field appears during manifest consumption:

| Unresolved item | Gate blocked while active |
|---|---|
| Engram wkv e_proj LR (1 versus 5) | F2-I text and multimodal |
| Engram q_weight/k_weight e_norm LR (1 versus 5) | F2-I text and multimodal |
| attn.wo_a exact grouped Muon lowering | F1/F2-I text and multimodal |
| Vision fused gate/up training grouping | F2-I unfrozen vision |
| Smaller encoder LR v after unfreeze | F3-I unfreeze/F2-I active encoder |
| vision.norm LR after unfreeze | F3-I transition/F2-I |
| image_start, image_end, image_newline algorithm, logical representation, LR and decay | F2-I whenever these vectors are trained |
| Quantized scale regeneration/master representation qualification | E1e/E2-S and F2-I quantized training |

Inactive vision OPENs need not block a declared pure-text stage; active Engram
and wo_a fields do. Each resolution records source or approved port deviation,
value, phase and affected owner families. Also retain separate B2/D6 indexer
objective/coefficient/detach unknowns and D5 load-reduction unknowns; they are
not resolved by A4 optimizer routing.

F3 consumes A4's pretraining schedule: 2000-step warmup to 2.6e-4, constant
through 28T tokens, cosine to 2.6e-5 over 28T–40T, constant to 45T; sequence
64K→1M at 34T. These are pretraining facts, not imposed RL defaults. Freeze the
encoder until LR decay starts, except final vision.norm; aligner remains
trainable. Image-vector groups require the ledger decision. Unfreeze creates
and synchronizes newly active optimizer state at the resolved smaller LR;
vision.norm's post-unfreeze LR requires its own resolution.
...
coordinates. Compare separately replayed segments to their concatenated replay
...
boundaries; sequence extension and pre/post-training quantization switches;
...
| 12 visual stages | 11 | F3 actual construction/sync/freeze exceptions/unfreeze and E5v restore |
...
| E3 | Preserve counts but swap two distinct consumer vectors/owners, or update shadow too; exact sum and owner update fail |
...
method dataflow exercised and eight mutations rejected without a hash gate.
