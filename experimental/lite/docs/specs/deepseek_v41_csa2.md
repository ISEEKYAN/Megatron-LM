...
The owner contract makes the existing official CED boundary `(h20, p20)`
explicit after Block(19); it is not a proposed new training architecture.
Layer 20 consumes `x20 = attn_norm_20(hc_pre(h20, p20))`, then its ratio-1
compressor produces `latent20 = norm(wkv(x20))`. Index K is `k_norm(wk(latent20))`
before main RoPE; main KV follows official RoPE/FP4 ordering and is shared by
layers 21–39. Each layer still projects its own SWA KV. Raw four-dimensional
h20 cannot bypass paired pre_mix or attention normalization. No new decoder
projection is needed. The full official loop provides a forward oracle for
this sharing, even though it does not optimize decoder prefill scheduling.
