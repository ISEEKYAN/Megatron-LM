...
Single-rank, DP=2, and EP=2 MLite runtime scratch training are assembled. Single-rank and DP validation pass; EP numerical acceptance awaits a reviewed tolerance. TP/ETP/CP/PP, full HF loading, and end-to-end external-reference parity remain unvalidated.
...
The registered protocol uses the existing MLite runtime and distributed optimizer. Set `load_hf_weights=False`; HF import/export and TP/ETP/CP/PP greater than one fail explicitly. `qwen38_dp_probe.py` and `qwen38_ep_probe.py` under `tests/smoke/workflows/training/` compare two distinct documents against a single rank with two microbatches. Post-fix validation at `818ea3b94`: 97 CPU tests pass, zero skipped; single-GPU job 18491270 passes 16 tests including 12 optimizer steps, all decoder branches, and actual FP32 expert-gradient low bits; DP job 18491286 passes three repeated runs with bitwise losses, reduced gradients, and updated parameters. Disabling fused wgrad in job 18491536 fails specifically at `EXPERT_WGRAD_BF16_TRUNCATED`. EP job 18491421 completes same-EP model/optimizer checkpoint restoration and bitwise continuation, but exits 1 at the final cross-EP bitwise check: only expert gradients/updates differ. Three repeated EP runs are individually deterministic; aligned expert inputs/backward signals yield identical FP64 wgrad oracles, identifying FP32 summation differences. Conditional EP proxy tolerance approval was revoked when removing the owner lookup guard still passed all 95 Qwen CPU tests at `355c2dd34`; permissive comparisons were reverted and the historical negative-test coverage gap requires correction and renewed review. Cross-EP-size checkpoint restoration is unvalidated. Full-suite job 18491796 at `818ea3b94` has 842 passed, 11 failed, five collection errors, 20 skipped and one xfailed; its failure/error set matches merge-base job 18488661 exactly (added=0, removed=0). The full-suite exit code is 1 for those existing baseline failures. Packed GDN requires FLA; full model, real-data, and external-reference parity remain unvalidated.
