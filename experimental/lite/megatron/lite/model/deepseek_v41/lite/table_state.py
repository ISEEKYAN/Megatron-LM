...
class EngramTableState:
    ...
    def accept_gradient(self, step, gradient):
        ...
        if self.main_grad is not None:
            if (gradient is None or gradient.dtype != torch.float32
                    or gradient.shape != self.main_grad.shape or gradient.device != self.main_grad.device):
    ...
    def step(self, update_rule, *, lr, beta=0.95, skip=False):
        ...
        if (not isinstance(direction, torch.Tensor) or direction.shape != self.main_grad.shape
                or direction.dtype != torch.float32 or direction.device != self.main_grad.device):
    ...
    def state_dict(self):
        ...
        state = {
            'format_version': 1, 'version': self.version, 'last_step': self.last_step,
        ...
        if self.table.master is not None:
            state.update(master=self.table.master.detach().clone(), momentum=self.momentum.clone())
        return state
    ...
    def load_state_dict(self, state):
        ...
        expected = {'format_version', 'version', 'last_step', 'trainable', 'weight', 'scale'}
        if self.table.master is not None:
        ...
        if (set(state) != expected or state['format_version'] != 1
                ...
                or type(state['version']) is not int or state['version'] < 0
                or type(state['last_step']) is not int or state['last_step'] < -1):
        ...
        for key, destination in destinations.items():
            source = state[key]
            if not isinstance(source, torch.Tensor) or source.dtype != destination.dtype or source.shape != destination.shape:
