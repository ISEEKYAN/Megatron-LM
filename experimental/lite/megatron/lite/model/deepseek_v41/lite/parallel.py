...
class EngramLayout:
    ...
    def __init__(self, rows, rank_groups, *, world_size, aliases=None):
        ...
        if (any(len(group) != width for group in self.rank_groups)
                or len(set(ranks)) != len(ranks)
                or any(not isinstance(rank, int) or rank < 0 or rank >= world_size for rank in ranks)):
            raise ValueError("Rank map must be rectangular with unique existing global ranks")
        self.rows = rows
