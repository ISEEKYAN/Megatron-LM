...
class _PrefetchedTable(nn.Module):
    ...
    def lookup_fp8(self, ids):
        ...
        if (
            ids.dtype != expected.dtype
            or ids.device != expected.device
            or not torch.equal(ids, expected)
        ):
            raise ValueError(
                'Cached microbatch IDs do not match the prefetched request'
            )
...
class EngramPrefetch:
    ...
    def start(self, step, microbatches, *, stream=None):
        ...
        if not microbatches:
            raise ValueError(
                'Require an explicit microbatch schedule, including empty batches'
            )
...
class PrefetchedBatch:
    def __init__(self, state, step, microbatches):
        ...
        for key, ids in self.ids.items():
            ...
            if self.source_master is not None:
                master = (
                    self.source_master[offset:end]
                    .detach()
                    .clone()
                    .reshape(*ids.shape, values.shape[-1])
                )
                master.requires_grad_()
                tag = GradientTag(step, self.version, key)
                master.register_hook(
                    lambda gradient, tag=tag: self.return_gradient(tag, gradient)
                )
            self.rows[key] = (
                values[offset:end].reshape(*ids.shape, values.shape[-1]),
                scales[offset:end].reshape(*ids.shape, scales.shape[-1]),
                master,
            )
            offset = end
    ...
    def return_gradient(self, tag, gradient):
        ...
        if (
            tag.step != self.step
            or tag.version != self.version
            or tag.microbatch not in self.ids
        ):
        ...
        if (
            ...
            or gradient.shape != master.shape
            or gradient.device != master.device
        ):
            raise ValueError(
                'Gradient return must match the resident FP32 microbatch rows'
            )
    ...
    def flush(self):
        ...
        if trainable:
            ...
            combined = torch.cat(
                [
                    self.returns[key].reshape(-1, self.source_master.shape[-1])
                    for key in self.ids
                ]
            )
            (gradient,) = torch.autograd.grad(
                self.source_master, self.state.table.master, combined
            )
