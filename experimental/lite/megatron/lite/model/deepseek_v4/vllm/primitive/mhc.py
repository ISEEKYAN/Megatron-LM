...
def mhc_post_pre(
    ...
    *,
    ...
    def functional(x_, residual_, post_, comb_, fn_, scale_, base_, norm_weight_):
        streams = _post_graph(x_, residual_, post_, comb_)
        new_residual, new_post, new_comb, hidden = _pre_graph(
            streams, fn_, scale_, base_, mult=mult, iters=iters, eps=eps
        )
        hidden = F.rms_norm(hidden, (hidden.shape[-1],), norm_weight_, norm_eps)
        return new_residual, new_post, new_comb, hidden

    return visible_functional_vjp(
        visible_op,
        functional,
        (x, residual, post, comb, fn, scale, base, norm_weight),
        version_indices=(4, 5, 6, 7),
    )
...
__all__ = ["mhc_head", "mhc_post", "mhc_post_pre", "mhc_pre_broadcast"]
