...
def _roll_mtp_left(
    ...
    *,
    ...
    DS4 inputs are dense ``[B, S]``. Under CP>1 the shared CP primitive carries
    the next-token target across the contiguous rank boundary.
    """
    if ps is not None and ps.cp_size > 1:
        return roll_contiguous_left_for_cp(
            tensor, cp_rank=ps.cp_rank, cp_size=ps.cp_size,
            cp_group=ps.cp_group, seq_dim=dims,
        )
...
class DeepseekV4Model(nn.Module):
    ...
    def forward(
        ...
        # fallback, mirroring Kimi). CP target rolls cross rank boundaries via
        # ``roll_contiguous_left_for_cp``.
        ...
        *,
        ...
        for mtp_hidden in mtp_hidden_states:
            ...
            mtp_loss_mask, num_tokens = _roll_mtp_left(
                mtp_loss_mask, ps=self.ps, dims=-1
            )
