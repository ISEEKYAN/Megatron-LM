...
def quantize_main_kv(post_rope):
    ...
    amax = blocks.abs().amax(-1).clamp_min(6 * 2.**-9)
    ...
    table = torch.tensor([0, .5, 1, 1.5, 2, 3, 4, 6], device=post_rope.device)
    decoded = table[(codes & 7).long()] * torch.where((codes & 8) != 0, -1., 1.)
