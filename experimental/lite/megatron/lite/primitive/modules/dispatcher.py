...
class _DeepEPCombineBackwardOnly(torch.autograd.Function):
    @staticmethod
    def forward(
        ctx,
        buffer,
        ...
        ctx.async_finish = async_finish
        ctx.allocate_on_comm_stream = allocate_on_comm_stream
        return rank_grouped.new_empty(output_shape)
...
class TokenDispatcher:
    ...
    def combine_deepep_backward_only(
        self,
        expert_output: torch.Tensor,
        ...
        *,
        ...
        if not self.use_deepep:
            raise RuntimeError("combine_deepep_backward_only requires DeepEP combine.")
        rank_grouped = unpermute(
            expert_output,
            self._row_id_map,
            restore_shape=self._restore_shape,
        ...
        combined = _DeepEPCombineBackwardOnly.apply(
            self.buffer,
            rank_grouped,
            self._handle,
            tuple(output_shape),
