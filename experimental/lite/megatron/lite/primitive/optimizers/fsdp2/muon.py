...
from emerging_optimizers.orthogonalized_optimizers.muon import (  # noqa: E402
...
def newton_schulz_orthogonalize(
    ...
    if x.ndim != 2:
        raise ValueError(
            f"newton_schulz_orthogonalize expects a 2D matrix, got {x.ndim}D"
        )
    if x.dtype != torch.float32:
        raise TypeError(
            f"newton_schulz_orthogonalize requires float32 input, got {x.dtype}"
        )
    if steps < 1:
...
class FP32Muon:
    ...
    def __init__(
        ...
        *,
        ...
        self.qkv_split_shapes = (
            list(qkv_split_shapes) if qkv_split_shapes is not None else None
        )
    ...
    def _model_param_dtype(self, param: nn.Parameter) -> torch.dtype | None:
        return self._model_dtype_for_param.get(id(param)) or fsdp2_model_param_dtype(
            param
        )
    ...
    def _gather_full(
        self, update_local: torch.Tensor, master: torch.Tensor
    ) -> torch.Tensor:
        if not is_dtensor_like(master):
            return update_local
    ...
    def _reshard_local(
        self, orth_full: torch.Tensor, master: torch.Tensor
    ) -> torch.Tensor:
        if not is_dtensor_like(master):
            return orth_full
    ...
    def _orthogonalize(
        ...
        if self.split_qkv and getattr(param, "is_qkv", False):
            split_shapes = (
                getattr(param, "qkv_split_shapes", None) or self.qkv_split_shapes
            )
            if not split_shapes:
                raise RuntimeError(
                    "Muon QKV split requested but qkv_split_shapes is unset."
                )
            full_update = self._gather_full(update_local, master)
            return self._reshard_local(
                self._orthogonalize_qkv(full_update, split_shapes), master
            )
        ...
        full_update = self._gather_full(update_local, master)
        return self._reshard_local(self._scaled_orthogonalize(full_update), master)
    ...
    def _scaled_orthogonalize_distributed(
        ...
        *,
        ...
        if partition_dim is None:
            orth = newton_schulz_orthogonalize(
                grad, self.num_ns_steps, self.coefficient_type
            )
        else:
            if tp_group is None:
                raise RuntimeError(
                    "DTensor shard placement requires a process group for Muon NS."
                )
    ...
    def _scaled_orthogonalize(self, grad: torch.Tensor) -> torch.Tensor:
        return self._scaled_orthogonalize_distributed(
            grad, tp_group=None, partition_dim=None
        )

    def _orthogonalize_qkv(
        self, grad: torch.Tensor, split_shapes: list[int]
    ) -> torch.Tensor:
    ...
    def state_dict(self) -> dict[str, Any]:
        return {
            ...
            "momentum_buffers": [
                self.state[param]["momentum_buffer"] for param in self.params
            ],
