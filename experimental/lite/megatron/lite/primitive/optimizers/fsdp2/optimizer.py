...
def build_fsdp2_adamw(
    ...
    *,
    ...
    optimizer_name = getattr(opt, "optimizer_algorithm", getattr(opt, "optimizer", "adam"))
...
def build_fsdp2_muon(
    ...
    *,
    ...
    _muon_only, _muon_groups, muon_names, muon_model_dtypes = _build_adamw_param_groups(
        model_chunks,
        weight_decay=float(getattr(opt, "weight_decay", 0.01)),
        apply_wd_to_qk_layernorm=bool(getattr(opt, "apply_wd_to_qk_layernorm", False)),
        model_param_dtypes=model_param_dtypes,
    ...
    fallback_params, fallback_groups, fallback_names, fallback_model_dtypes = _build_adamw_param_groups(
        model_chunks,
        weight_decay=float(getattr(opt, "weight_decay", 0.01)),
        apply_wd_to_qk_layernorm=bool(getattr(opt, "apply_wd_to_qk_layernorm", False)),
        model_param_dtypes=model_param_dtypes,
        include_param=lambda p: not bool(getattr(p, "is_managed_by_layer_wise_optimizer", False)),
    )
    muon_params, _, _ = split_muon_and_fallback_params(model_chunks)
...
def build_fsdp2_training_optimizer(
    ...
    *,
    ...
    is_muon = str(getattr(opt, "optimizer_algorithm", getattr(opt, "optimizer", "adam"))).lower() == "muon"
    if is_muon:
