...
def validate_dist_opt_config(engine_cfg) -> None:
    ...
    if str(opt.optimizer_algorithm).lower() != "muon":
        return
    ...
    if bool(_optimizer_value(opt, "use_precision_aware_optimizer", False)):
        raise ValueError("Muon does not support the Adam-only precision-aware optimizer.")

    offload_requested = any(
        (
            bool(_optimizer_value(opt, "optimizer_cpu_offload", False)),
            float(_optimizer_value(opt, "optimizer_offload_fraction", 0.0) or 0.0) > 0.0,
...
def build_dist_opt_optimizer_config(
    ...
    *,
    ...
    """
    optimizer_name = str(opt.optimizer_algorithm).lower()
    ...
    if override_optimizer_config:
        ...
        if "optimizer" in normalized_overrides:
            normalized_overrides["optimizer"] = str(normalized_overrides["optimizer"]).lower()
        lowering_owned = {
            "optimizer",
...
def build_dist_opt_stack(
    ...
    *,
    ...
    use_muon = str(opt.optimizer_algorithm).lower() == "muon"
    if use_muon and skip_ddp_wrap:
        raise ValueError("Muon×dist_opt requires unwrapped model chunks for buffer routing.")
    ...
    if skip_ddp_wrap:
        ...
        ddp_config = DistributedDataParallelConfig(
            ...
            overlap_grad_reduce=bool(_optimizer_value(opt, "overlap_grad_reduce", False)),
            overlap_param_gather=bool(_optimizer_value(opt, "overlap_param_gather", False)),
