...
"""Schedule-derived slot planning for chunk-wise CUDA graphs.

The FIFO allocator and ``ChunkCudaGraphSlotPlan`` contract are ported from
NVIDIA Megatron-LM PR #5258 (commit ``3b5dd928d``,
``megatron/core/transformer/chunk_cuda_graphs.py``). Capture/replay order is
**not** copied from MCore: it is derived from MLite's own pipeline schedules in
``primitive.parallel.pipeline`` (1F1B for ``len(model_chunks)==1``, interleaved
VPP for ``len(model_chunks)>1``). This module is config-only, pure-CPU, and has
no model / TE / CUDA dependency.
"""
...
class ChunkCudaGraphRuntimeSlots:
    ...
    def reset(self):
        ...
        self.live_slots_by_microbatch: dict[int, int] = {}
...
class ChunkCudaGraphSlotPlan:
    ...
    slot_ids: tuple[int | None, ...]
    chunk_ids: tuple[int | None, ...]
    op_types: tuple[str, ...]
    virtual_microbatch_ids: tuple[int | None, ...]
    microbatch_ids: tuple[int | None, ...]
    forward_slot_by_virtual_microbatch: tuple[int | None, ...]
    backward_slot_by_virtual_microbatch: tuple[int | None, ...]
    forward_slot_by_chunk_microbatch: tuple[tuple[int | None, ...], ...]
    backward_slot_by_chunk_microbatch: tuple[tuple[int | None, ...], ...]

    def get_forward_slot(self, chunk_id: int, microbatch_id: int) -> int | None:
    ...
    def get_backward_slot(self, chunk_id: int, microbatch_id: int) -> int | None:
...
def validate_cuda_graph_vpp_layout(
    vpp: int | None = None,
    vpp_chunk_id: int | None = None,
) -> None:
    """Fail loud when ``pp.py`` layer-layout VPP is requested (``pp.py:59-60``)."""
    if (vpp is not None and int(vpp) > 1) or vpp_chunk_id is not None:
        raise NotImplementedError(_VPP_LAYOUT_UNSUPPORTED_MSG)


def get_mlite_1f1b_warmup_microbatches(
    pp_size: int, pp_rank: int, num_microbatches: int
) -> int:
    """Warmup forward count for MLite ``_1f1b_schedule`` at ``(pp_size, pp_rank)``."""
    pp_size = int(pp_size)
    pp_rank = int(pp_rank)
    num_microbatches = int(num_microbatches)
    assert pp_size >= 1 and 0 <= pp_rank < pp_size
    assert num_microbatches >= 0
    return min(pp_size - pp_rank - 1, num_microbatches)


def build_mlite_1f1b_signed_order(
    pp_size: int, pp_rank: int, num_microbatches: int
) -> tuple[int, ...]:
    """Signed +chunk/-chunk order mirroring MLite ``_1f1b_schedule`` slot lifetime.

    With one local model chunk per PP rank the chunk id is always ``+1`` / ``-1``.
    """
    num_warmup = get_mlite_1f1b_warmup_microbatches(pp_size, pp_rank, num_microbatches)
    num_steady = num_microbatches - num_warmup
    order: list[int] = [1] * num_warmup
...
def build_mlite_vpp_signed_order_per_microbatch(
    pp_size: int,
    pp_rank: int,
    num_model_chunks: int,
) -> tuple[int, ...]:
    """Signed order for one microbatch on a PP rank.

    Mirrors MLite ``_interleaved_1f1b_schedule``: global virtual stages run forward
    in ascending order, then backward in descending order. Only stages owned by
    ``pp_rank`` appear in the local order.
    """
    pp_size = int(pp_size)
    pp_rank = int(pp_rank)
    num_model_chunks = int(num_model_chunks)
    assert pp_size >= 1 and 0 <= pp_rank < pp_size
    assert num_model_chunks >= 2, "use build_mlite_1f1b_signed_order for a single local chunk"

    total_stages = pp_size * num_model_chunks
    order: list[int] = []
    for stage_id in range(total_stages):
        if stage_id % pp_size != pp_rank:
            continue
        chunk_id = stage_id // pp_size
        order.append(chunk_id + 1)
    for stage_id in range(total_stages - 1, -1, -1):
        if stage_id % pp_size != pp_rank:
            continue
        chunk_id = stage_id // pp_size
        order.append(chunk_id - num_model_chunks)
    return tuple(order)


def build_mlite_vpp_signed_order(
    pp_size: int,
    pp_rank: int,
    num_model_chunks: int,
    num_microbatches: int,
) -> tuple[int, ...]:
    """Concatenate per-microbatch VPP orders for MLite's non-overlapped interleaved schedule."""
    per_mb = build_mlite_vpp_signed_order_per_microbatch(pp_size, pp_rank, num_model_chunks)
    order: list[int] = []
    for _ in range(int(num_microbatches)):
        order.extend(per_mb)
    return tuple(order)


def build_chunk_cuda_graph_slot_plan_from_mlite_1f1b(
    pp_size: int,
    pp_rank: int,
    num_microbatches: int,
    num_model_chunks: int = 1,
) -> ChunkCudaGraphSlotPlan:
    """Derive a slot plan from MLite's 1F1B pipeline schedule."""
    if num_model_chunks != 1:
        raise ValueError(
            "MLite 1F1B schedule uses one local chunk per PP rank; "
            f"use build_chunk_cuda_graph_slot_plan_from_mlite_vpp (got {num_model_chunks})."
        )
    order = build_mlite_1f1b_signed_order(pp_size, pp_rank, num_microbatches)
    num_warmup = get_mlite_1f1b_warmup_microbatches(pp_size, pp_rank, num_microbatches)
    stage_order = get_cuda_graph_schedule_stage_order_from_counts(num_warmup, num_microbatches)
    return build_chunk_cuda_graph_slot_plan(order, num_model_chunks=1, stage_order=stage_order)


def build_chunk_cuda_graph_slot_plan_from_mlite_vpp(
    pp_size: int,
    pp_rank: int,
    num_model_chunks: int,
    num_microbatches: int,
) -> ChunkCudaGraphSlotPlan:
    """Derive a slot plan from MLite's interleaved VPP pipeline schedule."""
    if num_model_chunks < 2:
        raise ValueError(
            "MLite interleaved VPP requires num_model_chunks >= 2; "
            f"use build_chunk_cuda_graph_slot_plan_from_mlite_1f1b (got {num_model_chunks})."
        )
    order = build_mlite_vpp_signed_order(pp_size, pp_rank, num_model_chunks, num_microbatches)
    stage_order = get_cuda_graph_schedule_stage_order(order)
    return build_chunk_cuda_graph_slot_plan(
        order, num_model_chunks=num_model_chunks, stage_order=stage_order
    )


def build_chunk_cuda_graph_slot_plan_from_mlite_pipeline(
    pp_size: int,
    pp_rank: int,
    num_microbatches: int,
    num_model_chunks: int = 1,
    *,
    vpp_layout: int | None = None,
    vpp_chunk_id: int | None = None,
) -> ChunkCudaGraphSlotPlan:
    """Top-level MLite planner: 1F1B or interleaved VPP from ``pipeline.py`` dispatch."""
    validate_cuda_graph_vpp_layout(vpp_layout, vpp_chunk_id)
    if num_model_chunks <= 1:
        return build_chunk_cuda_graph_slot_plan_from_mlite_1f1b(
            pp_size, pp_rank, num_microbatches, num_model_chunks=1
        )
    return build_chunk_cuda_graph_slot_plan_from_mlite_vpp(
        pp_size, pp_rank, num_model_chunks, num_microbatches
    )
...
def build_chunk_cuda_graph_slot_plan_from_schedule(
    ...
    """Build a slot plan while preserving virtual microbatch ids from a schedule table."""
    if stage_order is None:
...
__all__ = [
    ...
    "get_mlite_1f1b_warmup_microbatches",
