...
Ported verbatim from NVIDIA Megatron-LM PR #5258 (commit ``3b5dd928d``,
``megatron/core/transformer/chunk_cuda_graphs.py``). This module is config-only,
pure-CPU, and has no MCore/TE/CUDA dependency, so it is copied rather than
reinvented. It derives a per-chunk FIFO of live microbatch slots from a real
1F1B/VPP forward/backward schedule order; the resulting ``(chunk, slot)`` keys
index the captured forward/backward graph pairs owned by the CUDA Graph
controller. The runtime capture/replay itself is TE-backed (see
``primitive.cuda_graph``), so only this scheduling contract is reused.
"""
