    ...
    for layer in range(40):
        ...
        mode = (
            ...
            if layer < 2
                ...
                "Full"
                if layer in KV_OWNERS
                else "Reindex" if layer in INDEX_OWNERS else "Reuse"
            )
        )
        rows.append(
            dict(
                layer=layer,
                ratio=0 if layer < 2 else 2 if layer < 20 else 1,
                mode=mode,
                main_kv_owner=kv,
                index_k_owner=kv,
                selection_owner=index,
                local_kv_owner=layer,
...
def engram_layout():
    ...
    for _ in (1, 14):
        ...
        for _ in range(3):
            ...
            for _ in range(8):
                current += 1
                while current in used or any(
                    current % divisor == 0
                    for divisor in range(2, math.isqrt(current) + 1)
                ):
                    current += 1
                used.add(current)
...
def reduced_overrides():
    return dict(
        ...
        kv_source_layers=list(KV_OWNERS),
        index_source_layers=list(INDEX_OWNERS),
        ...
        engram_num_embeddings=[layer["rows"] for layer in engram_layout()],
        engram_pad_id=2,
        vision_n_layers=2,
        vision_dim=64,
        vision_n_heads=4,
...
def packed_case():
    ...
    return dict(
        lengths=lengths,
        cu_seqlens=offsets,
        input_ids=tokens + [2] * 3,
        sample_ids=[
            sample for sample, length in enumerate(lengths) for _ in range(length)
        ]
        + [-1] * 3,
...
def image_case():
    ...
    tokens = [
        255 if kind >= 0 else (13 * position + 3) % 254
        for position, kind in enumerate(types)
    ]
    return dict(
        input_ids=tokens,
        token_types=types,
        text_mask=[kind < 0 for kind in types],
        spans=[[3, 17], [19, 33]],
        patch_grids=[[9, 9], [9, 9]],
        patch_shape=[81, 3, 14, 14],
    )
...
def metadata():
    return dict(
        schema_version=1,
        profile="reduced-forward-v1",
        reduced=True,
        overrides=reduced_overrides(),
        owners=ownership_rows(),
        engram_layout=engram_layout(),
        weight_recipe="rational-v1",
        trainability="not_applicable",
        active_decisions=[],
        inactive_decisions={
            "O08": "no table update",
            "O09": "no master weights",
            "O12": "no indexer objective",
        },
        packed=packed_case(),
        images=image_case(),
        operator_lengths=[
...
def generate(reference_dir, output):
    ...
    args_map = map_release_config(
        json.loads((reference_dir / "config.json").read_text()),
        dataclasses.asdict(official.ModelArgs()),
    )
    unknown = source_config.keys() - args_map.keys()
    if unknown:
        raise ValueError(f"unknown official ModelArgs fields: {sorted(unknown)}")
    if any(args_map[key] != value for key, value in source_config.items()):
        raise ValueError(
            "explicit release mapping differs from pinned inference config"
        )
    args_map.update(reduced_overrides())
    args_map["dspark_block_size"] = (
        0  # recorded constructor-only waiver, never exported config
    )
    old_dtype = torch.get_default_dtype()
    torch.set_default_dtype(torch.bfloat16)
    try:
        with torch.device("cpu"):
            model = official.Transformer(
                official.ModelArgs(**args_map), tokenizer=IdentityTokenizer()
            )
    finally:
    ...
    for family in families:
        ...
        expected.update(
            pattern.format(*indices) for indices in itertools.product(*domains)
        )
    if names != expected:
        raise ValueError(
            f"reduced A2 names mismatch: missing={sorted(expected-names)} extra={sorted(names-expected)}"
        )
    ...
    for ordinal, name in enumerate(sorted(names)):
        ...
        if fp4:
            ...
            if name.endswith(".wo_a.weight"):
                converted[name] = dequantize_block_fp8(value, scale, block).to(
                    target.dtype
                )
            else:
                converted[name], converted[scale_name] = value, scale
    ...
    for length in metadata()["operator_lengths"]:
        if length <= 513:
            inputs[f"length{length}.input_ids"] = (
                torch.arange(length, dtype=torch.int64) * 13 + 3
            ) % 254
    ...
    manifest = metadata()
    manifest.update(
        effective_model_args=args_map,
        constructor_overrides={"dspark_block_size": 0},
        original_config=json.loads((reference_dir / "config.json").read_text()),
        tensors=records,
        converted_keys=sorted(converted),
        input_keys=sorted(inputs),
        expected_cards_sha256=hashlib.sha256(cards).hexdigest(),
        file_digests={
    ...
    print(
        f"REDUCED_FIXTURE_GENERATED layers=40 keys={len(release)} converted={len(converted)} digest={metadata_digest(manifest)}"
    )
    return manifest
