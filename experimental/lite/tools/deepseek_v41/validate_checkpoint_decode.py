...
def main():
    ...
    spec = importlib.util.spec_from_file_location("official_ds41_convert", args.official_convert)
    ...
    evidence = {"scope": "sampled-real-mtp-cpu-decode", "official_convert_sha256":
                hashlib.sha256(args.official_convert.read_bytes()).hexdigest(), "cases": []}
    for stage in range(3):
        for suffix in ("ffn.experts.0.w1.weight", "attn.wkv.weight", "attn.kv_norm.weight"):
            ...
            if scale_name in index:
                with safe_open(args.checkpoint / index[scale_name], framework="pt", device="cpu") as reader:
                ...
                reference = (weight.float().view(rows // 32, 32, columns // 32, 32)
                             * scale.float()[:, None, :, None]).reshape(rows, columns)
            else:
            ...
            evidence["cases"].append(dict(name=name, shape=list(result.shape), max_abs_error=error,
                                          bf16_roundtrip_max_abs_error=0.0))
    (args.output / "evidence.json").write_text(json.dumps(evidence, indent=2) + "\n")
    print(f"REAL_MTP_CPU_DECODE_OK cases={len(evidence['cases'])} max_abs_error=0 bf16_reload_error=0")
