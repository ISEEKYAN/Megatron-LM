...
from megatron.lite.primitive.quantization.ds41_fp8 import dynamic_fp8_linear, quantize_swa
...
def main():
    ...
    if args.include_fp8:
        for name, x in cases.items():
            ...
            decoded = kernel.act_quant(x.clone(), 32, "ue8m0", torch.float8_e8m0fnu, True)
            actual = quantize_swa(x)
            assert torch.equal(encoded.view(torch.uint8), actual.values.view(torch.uint8)), (name, "FP8 codes")
            assert torch.equal(scale.view(torch.uint8), actual.scale.view(torch.uint8)), (name, "FP8 scales")
            assert torch.equal(decoded, actual.decoded), (name, "FP8 decoded")
        torch.set_default_dtype(torch.bfloat16)
        x = cases["random"].clone().requires_grad_()
        weight = torch.randn(128,64, device="cuda", dtype=torch.bfloat16).requires_grad_()
        ...
        ref_x = (a.float().reshape(32,2,32) * sa.float()[:,:,None]).reshape(32,64).bfloat16()
        ref_w = (b.float().reshape(4,32,2,32) * sb.float()[:,None,:,None]).reshape(128,64).bfloat16()
        assert torch.equal(x.grad, (grad.float() @ ref_w.float()).bfloat16()), "input derivative"
        assert torch.equal(weight.grad, (grad.float().T @ ref_x.float()).bfloat16()), "weight derivative"
        print(f"OFFICIAL_FP8_GPU_OK activation_cases={len(cases)} gemm_max_error={error} derivatives=2 job={os.environ['SLURM_JOB_ID']}")
