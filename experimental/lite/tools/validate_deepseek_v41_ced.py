...
def method(source, cls, name, namespace, stop_before=None):
    ...
    if stop_before:
        # Execute the actual prefix; only expensive downstream scoring is excluded.
            ...
            and any(isinstance(t, ast.Name) and t.id == stop_before for t in n.targets)
        )
...
def check_dataflow(source):
    ...
    loop_ns = dict(
        ns, make_identity_pre_mix=lambda h, n: torch.zeros(1), sample=lambda x, t: x
    )
    ...
    model = NS(
        engram_hash=None,
        embed=lambda ids: torch.zeros(1, 3, 4),
        hc_mult=2,
    ...
    attn_p = torch.tensor([0.8, 0.2]).expand(1, 3, 2)
    ffn_p = torch.tensor([0.6, 0.4]).expand(1, 3, 2)
    mixes = iter((attn_p, ffn_p))
    compressor = NS(compress_ratio=1, wkv=lambda x: x * 3, norm=lambda x: x + 5)
    idx = NS(
        freqs_cis=torch.zeros(3),
        compress_ratio=1,
        rope_head_dim=4,
        owns_k=True,
        wk=lambda x: x * 5,
        k_norm=lambda x: x + 11,
    ...
    owner = NS(
        compress_ratio=1,
        is_kv_source=True,
        compressor=lambda x, pos: compress(compressor, x, pos),
        compress_kv_cache=torch.zeros(1, 3, 4),
        freqs_cis=torch.zeros(3),
        rope_head_dim=4,
    ...
    def ffn(x, mask):
        torch.testing.assert_close(x, (h * attn_p.unsqueeze(-1)).sum(2))
        return x

    b = NS(
        hc_mixes=lambda *a: (next(mixes), None, None),
        hc_pre=lambda x, pre: hc_pre(None, x, pre),
        attn_norm=lambda x: x + 13,
        attn=attention,
        hc_post=lambda x, residual, *a: residual,
        ffn_norm=lambda x: x,
        ffn=ffn,
    )
    for name in (
        ...
        setattr(b, name, None)
    result_h, result_p = block(b, h, 0, p, None)
    torch.testing.assert_close(result_h, h)
    torch.testing.assert_close(result_p, ffn_p)
    ...
    for layer in range(21, 40):
        ...
        consumer.compress_kv_cache = torch.full((1, 3, 4), -999.0)
        consumer.compressor = lambda *a: (_ for _ in ()).throw(
            AssertionError('consumer recompressed')
        )
        local_x = expected_x + layer
        swa = NS(
            window_size=128,
            kv_norm=lambda x: x + 17,
            wkv=lambda x: x * 2,
            rope_head_dim=4,
            window_kv_cache=torch.zeros(1, 128, 4),
        )
...
def validate(source):
    ...
    mutations = [
        ('x = self.hc_pre(x, pre_mix)', 'x = self.hc_pre(x, attn_pre)'),
        ('x = self.hc_pre(x, attn_pre)', 'x = self.hc_pre(x, ffn_pre)'),
        ('return x, ffn_pre', 'return x, attn_pre'),
        (
    ...
    print(
        f'ok: official CPU dataflow probes; {len(mutations)} rejected mutations (without hash gate)'
    )
