...
OPTIMIZER_ALGORITHM="${OPTIMIZER_ALGORITHM:-adamw}"
...
COMMON_ARGS=(
  ...
  # VERL calls this ``optimizer``; MegatronLiteEngine lowers it immediately
  # into the canonical runtime ``optimizer_algorithm`` contract.
  "optim.optimizer=${OPTIMIZER_ALGORITHM}"
  "+optim.override_optimizer_config.muon_momentum=${MUON_MOMENTUM}"
  "+optim.override_optimizer_config.muon_split_qkv=${MUON_SPLIT_QKV}"
  "+optim.override_optimizer_config.muon_nesterov=${MUON_NESTEROV}"
  "+optim.override_optimizer_config.muon_scale_mode=${MUON_SCALE_MODE}"
  "+optim.override_optimizer_config.muon_fp32_matmul_prec=${MUON_FP32_MATMUL_PREC}"
  "+optim.override_optimizer_config.muon_coefficient_type=${MUON_COEFFICIENT_TYPE}"
  "+optim.override_optimizer_config.muon_num_ns_steps=${MUON_NUM_NS_STEPS}"
  "+optim.override_optimizer_config.muon_tp_mode=${MUON_TP_MODE}"
  "+optim.override_optimizer_config.muon_extra_scale_factor=${MUON_EXTRA_SCALE_FACTOR}"
  "+optim.override_optimizer_config.muon_scalar_optimizer=${MUON_SCALAR_OPTIMIZER}"
