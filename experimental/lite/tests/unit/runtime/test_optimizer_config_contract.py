...
def test_muon_contract_matches_pinned_megatron_defaults() -> None:
    config = OptimizerConfig(optimizer_algorithm="muon")
...
def test_muon_scalar_optimizer_is_currently_adam_only() -> None:
    with pytest.raises(ValueError, match="muon_scalar_optimizer.*adam"):
        OptimizerConfig(optimizer_algorithm="muon", muon_scalar_optimizer="lion")
...
def test_muon_rejects_cross_step_param_gather_overlap() -> None:
    with pytest.raises(ValueError, match="overlap_param_gather_with_optimizer_step"):
        OptimizerConfig(
            optimizer_algorithm="muon", overlap_param_gather_with_optimizer_step=True
        )


def test_optimizer_algorithm_is_unambiguous_and_defaults_to_adam() -> None:
    assert OptimizerConfig().optimizer_algorithm == "adam"
