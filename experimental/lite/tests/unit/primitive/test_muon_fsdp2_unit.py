...
# ---------------------------------------------------------------------------
def _orthogonalize_full(
    update, *, steps, coeff, scale_mode, extra, split_shapes, matmul_prec
):
    with _fp32_matmul_precision(matmul_prec):
        ...
        return (
            o * muon_update_scale(update.size(-2), update.size(-1), scale_mode) * extra
        )
...
def _shard_worker(
    rank, world_size, store_path, rows, cols, num_steps, split_shapes, matmul_prec
):
    ...
    try:
        ...
        param = torch.nn.Parameter(
            distribute_tensor(full_weight.clone(), mesh, [placement])
        )
        if split_shapes:
            param.is_qkv = True
...
def _bounded_drift_worker(rank, world_size, store_path):
    ...
    try:
        ...
        assert max_diff < 5e-2, (
            f"rank {rank}: unbounded distributed-NS drift {max_diff}"
        )
        ...
        assert max_diff > 1e-4, (
            f"rank {rank}: unexpectedly tight at medium ({max_diff})"
        )
    finally:
        dist.destroy_process_group()
...
def _distributed_ns_worker(rank, world_size, store_path):
    ...
    try:
        ...
        def _forbid_all_gather(*args, **kwargs):
            raise AssertionError(
                "redundant all_gather called during distributed Muon NS"
            )

        def _forbid_full_tensor(self):
            raise AssertionError(
                "redundant full_tensor gather called during distributed Muon NS"
            )
...
def _build_muon_adam_facade(weight_init, norm_init):
    ...
    muon = FP32Muon(
        [{"params": [weight], "weight_decay": 0.02}],
        lr=0.1,
        momentum=0.9,
        weight_decay=0.02,
    )
