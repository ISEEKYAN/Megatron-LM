...
def test_lora_config_aliases_and_trainable_param_accounting():
    cfg = normalize_lora_spec({"enabled": True, "rank": 2, "alpha": 6, "targets": ["qkv", "fc2"]})
...
def test_lora_scaling_rslora():
    assert lora_scaling(4, 8, use_rslora=False) == 2.0
    assert lora_scaling(4, 8, use_rslora=True) == 4.0

    layer = LinearLoRA(3, 2, rank=4, alpha=8, dropout=0.0, use_rslora=True)
    assert layer.scale == 4.0
