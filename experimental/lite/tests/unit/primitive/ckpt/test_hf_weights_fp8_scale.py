...
def test_explicit_block_shape_preserves_partial_tail():
    from megatron.lite.primitive.ckpt.hf_weights import _dequantize_block_scaled_tensor

    weight = torch.ones(5, 7).to(torch.float8_e4m3fn)
    scale = torch.tensor([[1.0, 2.0], [3.0, 4.0]])
    expected = scale.repeat_interleave(4, 0).repeat_interleave(4, 1)[:5, :7]
    actual = _dequantize_block_scaled_tensor(
        weight, scale, weight.shape, block_shape=(4, 4)
    )
    assert torch.equal(actual, expected), 'EXPLICIT_BLOCK_WIDTH'
    with pytest.raises(ValueError, match='scale shape mismatch'):
        _dequantize_block_scaled_tensor(
            weight, scale[:1], weight.shape, block_shape=(4, 4)
        )
    with pytest.raises(ValueError, match='positive size'):
        _dequantize_block_scaled_tensor(weight, scale, weight.shape, block_shape=(0, 4))
