...
def test_pp_export_streams_over_nccl_and_matches_materialized(monkeypatch) -> None:
    ...
    assert torch.equal(exported["weight"].cpu(), local_weight)
    assert torch.equal(exported["router_bias"].cpu(), torch.tensor([1.0, 2.0]))
    assert torch.equal(exported["weight2"].cpu(), remote_weight)
    assert torch.equal(exported["router_bias2"].cpu(), remote_bias)
...
def test_pp_export_never_materializes_the_whole_stage(monkeypatch) -> None:
    ...
    assert torch.equal(first_tensor.cpu(), params["weight_a"])
