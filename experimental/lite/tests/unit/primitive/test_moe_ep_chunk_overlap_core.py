...
def test_expert_chunk_merge_and_split_reuse_supplied_storage(
    transformer_engine_import_stub,
):
    transformer_engine_import_stub()
    from megatron.lite.primitive.modules.moe_ep_chunk_overlap import (
    ...
    merged_storage = torch.empty(7, 1, dtype=torch.int64)
    split_storage = [torch.empty_like(chunk) for chunk in chunks]

    merged = _merge_expert_chunks(chunks, counts, out=merged_storage)
    restored = _split_expert_chunks(merged, counts, out=split_storage)

    assert merged.data_ptr() == merged_storage.data_ptr()
    assert all(
        actual.data_ptr() == expected.data_ptr()
        for actual, expected in zip(restored, split_storage, strict=True)
    )
    torch.testing.assert_close(restored[0], chunks[0])
    torch.testing.assert_close(restored[1], chunks[1])
