...
def _build_muon_fsdp2_model(dtype: torch.dtype = torch.bfloat16) -> tuple[nn.Module, ParallelState]:
...
def _build_muon_optimizer(model: nn.Module, ps: ParallelState, *, offload_fraction: float = 0.0):
    return build_fsdp2_muon([model], _muon_opt(offload_fraction), ps, use_fp32_master=True)
...
def test_muon_offload_reload_then_resume_matches_uninterrupted_single_gpu():
    ...
    handle = ModelHandle(
        model=run_model, optimizer=run_opt, parallel_state=run_ps,
    ...
    for name in ref_params:
        torch.testing.assert_close(run_params[name], ref_params[name], atol=0.0, rtol=0.0)
