...
def test_packed_replay_whole_checkpoint_fifo_and_mask():
    from megatron.lite.primitive.modules.router_replay import (
        PackedRouterReplay,
        RouterReplay,
        RouterReplayAction,
    ...
    replay = RouterReplay()
    try:
        a, b = torch.arange(10).reshape(5, 2), torch.arange(10).reshape(5, 2) + 20
        mask = torch.tensor([True, False, True, False, True])
        native = torch.full((5, 2), 99)
        for target in (a, b):
            RouterReplay.set_replay_data([target], mask)
            RouterReplay.set_global_router_replay_action(
                RouterReplayAction.REPLAY_FORWARD
            )
            packed = PackedRouterReplay(5)
            actual = []
            for lo, hi in ((0, 2), (2, 5)):
                with packed.sequence(lo, hi):
                    actual.append(replay.select_indices(native[lo:hi]))
            packed.finish()
            assert torch.equal(
                torch.cat(actual), torch.where(mask[:, None], target, native)
            ), 'G2_PACKED_REPLAY_BOUNDARY: target/mask moved across a sequence boundary'
        RouterReplay.set_global_router_replay_action(RouterReplayAction.REPLAY_BACKWARD)
        for target in (a, b):
            packed = PackedRouterReplay(5)
            actual = []
            ...
            assert torch.equal(
                torch.cat(actual), torch.where(mask[:, None], target, native)
            ), 'G2_CHECKPOINT_FIFO: packed recomputation consumed another microbatch target'
        assert not replay.replay_backward_list
    finally:
...
def test_checkpoint_replay_snapshots_survive_reverse_segments_and_new_microbatch(
    record,
):
    from megatron.lite.primitive.modules.router_replay import (
        PackedRouterReplay,
    ...
    replay = RouterReplay()
    try:
        dense = torch.arange(20, dtype=torch.float32).reshape(5, 4).requires_grad_()
        target = torch.tensor([[0, 1], [1, 2], [2, 3], [3, 0], [0, 2]])
        native = torch.tensor([[3, 2]]).expand(5, -1)
        mask = torch.tensor([1, 0, 1, 1, 0], dtype=torch.bool)
        RouterReplay.set_replay_data([target], mask)
        RouterReplay.set_global_router_replay_action(
            RouterReplayAction.RECORD if record else RouterReplayAction.REPLAY_FORWARD
        )
        packed, outputs, visits = PackedRouterReplay(5), [], []
        for lo, hi in ((0, 2), (2, 5)):

            def forward(x, lo=lo, hi=hi):
                visits.append((lo, hi))
                indices = replay.select_indices(native[lo:hi])
                return x.gather(1, indices).square()
            ...
            with packed.sequence(lo, hi):
                outputs.append(
                    checkpoint(
                        ...
                        dense[lo:hi],
                        use_reentrant=False,
                        context_fn=router_replay_checkpoint_contexts,
                    )
                )
        packed.finish()
        # A newer microbatch and an in-place producer write must not alter snapshots.
        RouterReplay.set_replay_data([torch.zeros_like(target)], ~mask)
        target.fill_(3)
        RouterReplay.set_global_router_replay_action(RouterReplayAction.REPLAY_BACKWARD)
        for output in reversed(outputs):
            output.sum().backward()
        selected = (
            native if record else torch.tensor([[0, 1], [3, 2], [2, 3], [3, 0], [3, 2]])
        )
        expected = torch.zeros_like(dense).scatter_add_(
            1, selected, 2 * dense.detach().gather(1, selected)
        )
        assert torch.equal(
            dense.grad, expected
        ), 'G2_CHECKPOINT_GRADIENT: recomputation used a different segment or microbatch route'
        assert (
            visits.count((0, 2)) == 2 and visits.count((2, 5)) == 2
        ), 'G2_CHECKPOINT_REACHABILITY: both segment forwards must actually recompute'
    finally:
        RouterReplay.clear_global_state()
        RouterReplay.clear_global_router_replay_instances()
