...
_SPEC = importlib.util.spec_from_file_location(
    "ds41_fixture_metadata", _ROOT / "tools/deepseek_v41/fixtures.py"
)
...
def test_40_layer_owner_identities():
    ...
    for row in rows[20:]:
        assert (
            row["main_kv_owner"] == row["index_k_owner"] == row["candidate_owner"] == 20
        )
        assert row["local_kv_owner"] == row["layer"]
    assert [
        rows[i]["selection_owner"] for i in (20, 23, 24, 27, 28, 31, 32, 35, 36, 39)
...
def test_primes_independent_primality_check():
    ...
    for layer in layouts:
        ...
        assert all(
            offset + p == following
            for offset, p, following in zip(
                layer["offsets"],
                layer["primes"],
                layer["offsets"][1:] + [layer["rows"]],
            )
        )
...
def test_weight_recipe_hand_known_sentinels_and_reproducibility():
    ...
    assert fixtures.metadata_digest(fixtures.metadata()) == fixtures.metadata_digest(
        fixtures.metadata()
    )
...
def test_packed_positions_and_images():
    ...
    assert all(
        token == 255
        for token, kind in zip(image["input_ids"], image["token_types"])
        if kind >= 0
    )
