...
def test_main_midpoints_codes_scales_and_sign():
    ...
    expected = torch.tensor([[codes[i] | (codes[i+1] << 4) for i in range(0,16,2)]], dtype=torch.uint8)
    ...
    assert result.decoded.tolist() == [[0., 1., 1., 2., 2., 4., 4., 6., -0., -1., -1., -2., -2., -4., -4., -6.]]
...
def test_zero_saturation_and_gradient():
    ...
    result = quantize_main_kv(x)
    assert torch.equal(result.decoded, torch.full_like(x, 6.))
