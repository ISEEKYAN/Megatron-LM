...
def test_fp4_known_codes_loaded_gemm_and_bf16_reload(tmp_path):
    packed = torch.tensor(list(bytes.fromhex("10 32 54 76 98 ba dc fe") * 2), dtype=torch.uint8)
    ...
    row = [0, .5, 1, 1.5, 2, 3, 4, 6, -0., -.5, -1, -1.5, -2, -3, -4, -6] * 2
...
def test_engram_row_scales_are_not_matrix_block_scales(tmp_path):
    ...
    scale = torch.tensor([[126], [127], [128]], dtype=torch.uint8).view(torch.float8_e8m0fnu)
    store = store_tensors(tmp_path, {name: weight, name[:-6] + "scale": scale})
    expected = torch.tensor([.5, 1, 2])[:, None].expand(3, 32)
...
def test_quantized_scale_failures(tmp_path, kind):
    ...
    if kind != "missing":
        scales = torch.tensor([[127], [127]], dtype=torch.uint8).view(torch.float8_e8m0fnu)
        if kind == "wrong_shape":
            scales = scales[:1]
...
def test_plain_weight_rejects_stale_scale(tmp_path):
    name = "norm.weight"
    store = store_tensors(tmp_path, {name: torch.ones(4, dtype=torch.bfloat16), "norm.scale": torch.ones(1)})
