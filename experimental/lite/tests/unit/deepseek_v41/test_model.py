...
import pytest
import torch
from megatron.lite.model.deepseek_v41.lite import engram, image_data, protocol
...
def test_staged_training_and_restart(moe, model_config, trainable_engram):
    ...
    batch = PackedBatch(
        ids, ids, torch.tensor([6]), torch.ones(6), extras={'images': [[image]]}
    )
    for bundle in (actual, serial):
        run_microbatch_loop(
            bundle.chunks[0], iter([batch, batch]), 2, bundle.forward_step
        )
    for p, q in zip(actual.chunks[0].parameters(), serial.chunks[0].parameters()):
        ...
        if p.grad is not None:
            torch.testing.assert_close(
                p.grad, q.grad, atol=0, rtol=0, msg='staged gradient'
            )
    ...
    for bundle in (actual, resumed):
        ...
        bundle.optimizer.reconfigure_vision(
            protocol.VisionTrainability(False, True, True, False)
        )
