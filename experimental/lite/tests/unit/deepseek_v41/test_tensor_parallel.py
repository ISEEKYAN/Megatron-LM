...
def _worker(rank, config, directory, case, trainable=False):
    ...
    impl = protocol.ImplConfig(
        device=f'cuda:{rank}', dtype=torch.float32, quantized=False,
        token_map=list(range(256)), trainable_engram=trainable,
        optimizer='muon', optimizer_config=OptimizerConfig(0.001, 5, 'quintic'),
    )
    # Independent full-matrix baseline exists before any process group.
    serial = protocol.build_model(config, impl_cfg=impl)
    restored = protocol.build_model(config, impl_cfg=impl) if case == 'training' else None
    dist.init_process_group(
        'nccl', init_method=(Path(directory) / 'rendezvous').as_uri(),
        rank=rank, world_size=2, timeout=timedelta(seconds=120),
    )
    try:
        parallel = protocol.build_model(config, impl_cfg=replace(impl, parallel=ParallelConfig(tp=2)))
        model, reference = parallel.chunks[0], serial.chunks[0]
        full_state = reference.state_dict()
        parameters = dict(model.named_parameters())
        model.load_state_dict({
            name: slice_parameter(parameters[name], value) if name in parameters else value
        ...
        assert model.ps.tp_size == 2 and model.ps.dp_size == 1 and model.ps.etp_size == 1
        print(f'TP_STORAGE rank={rank} local={local_count} serial={serial_count}', flush=True)
        if case == 'scope':
    ...
    for step in range(2):
        ...
        for name in names:
            torch.testing.assert_close(
                local[name], slice_parameter(local[name], full[name]), atol=0, rtol=0,
...
def test_tp_training_and_checkpoint_roundtrip(model_config, tmp_path, trainable):
    ...
    mp.spawn(_worker, args=(model_config, str(tmp_path), 'training', trainable), nprocs=2, join=True)
...
def _training(serial, parallel, restored, directory, trainable):
    ...
    for step in range(2):
        ...
        torch.testing.assert_close(actual, expected, atol=0, rtol=0, msg='TP_LOGITS_PARITY')
        for bundle in (serial, parallel):
            bundle.optimizer.zero_grad()
            run_microbatch_loop(
                bundle.chunks[0], iter([batch]), 1, bundle.forward_step,
        ...
        for name, p in model.named_parameters():
            ...
            if p.grad is not None:
                torch.testing.assert_close(p.grad, slice_parameter(p, q.grad), atol=0, rtol=0,
                                           msg='TP_GRADIENT_PARITY:' + name)
        assert serial.optimizer.step()[0]
        assert parallel.optimizer.step()[0]
        for name, p in model.named_parameters():
            torch.testing.assert_close(p, slice_parameter(p, reference_parameters[name]), atol=0, rtol=0,
                                       msg='TP_STEP_PARAMETER_PARITY:' + name)
    print(f'TP_TRAINING trainable_engram={trainable} two_steps logits=0 gradient=0 parameter=0', flush=True)
...
def _roundtrip(serial, parallel, restored, directory):
    from megatron.lite.model.deepseek_v41.lite.checkpoint import (
        CheckpointTensorStore, load_model, save_model,
    )
    from megatron.lite.primitive.parallel.matrix import slice_parameter
    ...
    if dist.get_rank() == 0:
        ...
        save_file({name: torch.zeros(1) for name in reference.archival_bindings}, archive)
    dist.barrier()
    ...
    for name, value in reference.state_dict().items():
        ...
        assert torch.equal(value.contiguous().view(torch.uint8), actual.contiguous().view(torch.uint8)), (
    ...
    for name, parameter in model.named_parameters():
        torch.testing.assert_close(parameter, slice_parameter(parameter, expected[name]),
                                   atol=0, rtol=0, msg='TP_CHECKPOINT_SINGLE_SLICE:' + name)
