...
``compose_kernel.assemble`` with a fake :class:`ModelSpec` and fake chunks, so
...
def _make_spec(monkeypatch, **spec_overrides):
    from megatron.lite.model import compose_kernel
    from megatron.lite.primitive.parallel import ParallelState

    # Keep assemble on CPU: no real parallel init, no CUDA.
    monkeypatch.setattr(compose_kernel, "init_parallel", lambda _cfg: ParallelState())
    ...
    spec = compose_kernel.ModelSpec(**spec_kwargs)
    return compose_kernel, spec, events


def test_assemble_single_chunk_bundle(monkeypatch):
    compose_kernel, spec, _ = _make_spec(monkeypatch)
    fwd = spec.forward_step
    bundle = compose_kernel.assemble(spec, model_cfg=SimpleNamespace(), impl_cfg=_fake_impl_cfg())
...
def test_assemble_vpp_builds_one_chunk_per_stage(monkeypatch):
    ...
    bundle = compose_kernel.assemble(spec, model_cfg=SimpleNamespace(), impl_cfg=impl)
    assert [c.chunk_id for c in bundle.chunks] == [0, 1, 2]


def test_prepare_and_post_chunk_hooks_ordering(monkeypatch):
    compose_kernel, _, _ = _make_spec(monkeypatch)
    order = []

    compose_kernel_mod, spec, _ = _make_spec(
        monkeypatch,
    ...
    spec = compose_kernel_mod.ModelSpec(
        **{**{f.name: getattr(spec, f.name) for f in spec.__dataclass_fields__.values()},
           "chunk_factory": tracking_factory}
    )
    compose_kernel_mod.assemble(spec, model_cfg=SimpleNamespace(), impl_cfg=_fake_impl_cfg())
...
def test_recompute_offload_skipped_when_empty(monkeypatch):
    ...
    monkeypatch.setattr(compose_kernel, "parse_recompute_spec", lambda names: [])
    monkeypatch.setattr(
        compose_kernel, "apply_recompute", lambda *a: called.__setitem__("recompute", called["recompute"] + 1)
    )
    monkeypatch.setattr(
        compose_kernel, "apply_offload", lambda *a: called.__setitem__("offload", called["offload"] + 1)
    )
    compose_kernel.assemble(spec, model_cfg=SimpleNamespace(), impl_cfg=_fake_impl_cfg())
...
def test_optimizer_backend_name_normalizer(monkeypatch):
    ...
    bundle = compose_kernel.assemble(
...
def test_wire_optimizer_applies_normalizer_before_dispatch(monkeypatch):
    ...
    opt, finalize, hook, backend = compose_kernel._wire_optimizer(
...
def test_pre_forward_hook_factory_populates_extras(monkeypatch):
    sentinel = object()
    compose_kernel, spec, _ = _make_spec(
