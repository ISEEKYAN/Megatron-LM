...
def test_qwen2_protocol_builds_tiny_lora_forward_backward():
    ...
    unpacked = unpack_forward_output(model, batch, output["log_probs"])
    assert unpacked.is_nested  # jagged per-sequence rows (verl postprocess contract)
    assert [r.numel() for r in unpacked.unbind()] == [3, 2]
    loss = output["loss"]
