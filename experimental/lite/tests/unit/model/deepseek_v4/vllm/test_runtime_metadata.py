...
def test_batch_invariance_initialization_is_noop_when_disabled(monkeypatch) -> None:
    lookup = Mock()
    monkeypatch.setattr(
        runtime.importlib,
        "import_module",
        lambda name: (
            SimpleNamespace(VLLM_BATCH_INVARIANT=False)
            if name == "vllm.envs"
...
def test_layer0_prefill_metadata_exact_contract(monkeypatch) -> None:
    ...
    assert metadata.flash_kind == "prefill"
    assert metadata.tile_scheduler_metadata is None
...
def test_layer0_decode_metadata_exact_contract(monkeypatch) -> None:
    scheduler = object()
    get_metadata = Mock(return_value=(scheduler, None))
    monkeypatch.setattr(runtime, "_symbol", lambda _module, _name: get_metadata)
    builder = runtime.DS4SparseAttentionMetadataBuilderAdapter(
        _config(), device="cpu", cos_sin_cache=torch.empty(256, 128)
    )
    metadata = builder.build_decode(130)

    assert metadata.positions.tolist() == [129]
    assert metadata.slot_mapping.tolist() == [129]
    assert metadata.swa_cache.shape == (1, 256, 584)
    assert metadata.indices.shape == (1, 1, 128)
    assert metadata.indices[0, 0].tolist() == list(range(2, 130))
    assert metadata.topk_length.tolist() == [128]
    assert metadata.output.shape == (1, 64, 512)
    assert metadata.tile_scheduler_metadata is scheduler
    assert metadata.prepare_flash is None
